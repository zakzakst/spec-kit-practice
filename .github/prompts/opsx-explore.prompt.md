...
- `これなら change を開始できそうです。作成しましょうか？`
...
- **実装しない**: アプリケーションコードは書かない
- **理解したふりをしない**: 不明なら深掘りする
- **急がない**: これは実装時間ではなく思考時間
- **構造を強制しない**: 自然な形を優先する
- **自動で保存しない**: 保存は提案にとどめる
- **可視化する**: 良い図は長い説明より価値がある
- **コードベースを調べる**: 現実のコードに根差す
- **前提を疑う**: ユーザーの前提も自分の前提も疑う
