...
**Specs:** main specs に同期済み

すべての成果物が完了しています。すべてのタスクが完了しています。
```

**成功時の出力（delta specs なし）**
...
ターゲットの archive ディレクトリは既に存在します。

**選択肢:**
1. 既存の archive をリネームする
2. 重複であれば既存の archive を削除する
3. 別の日付になるまで待って archive する
```
